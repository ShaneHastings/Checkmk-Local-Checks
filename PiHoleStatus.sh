...
if [[ $(pihole status | grep "enabled") ]]; then
    echo '0 "PiHole DNS  Service" - Pi-Hole blocking is enabled on the network.'
else
    echo '2 "PiHole DNS Service" - Pi-Hole is NOT enabled and DNS requests are not being processed.'
fi
